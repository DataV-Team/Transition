...
function getFrameStateProgressByCurve(curve, frameNum) {
    ...
    var tState = new Array(frameNum).fill(0).map(function (_, i) { return i * tGap; });
    return tState.map(function (t) { return getFrameProgressByT(targetCurve, t); });
}
/**
...
var easeInElastic = function (t, b, c, d) {
    ...
    return (-(c *
        Math.pow(2, 10 * (t -= 1)) *
        Math.sin(((t * d - (d * 0.3) / 4) * (2 * Math.PI)) / (d * 0.3))) + b);
};
var easeOutElastic = function (t, b, c, d) {
    ...
    if ((t /= d) === 1)
        return b + c;
    return (c * Math.pow(2, -10 * t) * Math.sin(((t * d - (d * 0.3) / 4) * (2 * Math.PI)) / (d * 0.3)) +
        c +
        b);
};
...
function getFrameStateProgressByTween(tweenName, frameNum) {
    ...
    return new Array(frameNum)
        .fill(0)
        .map(function (_, i) { return tweenFun(i * tGap, startState, changeValue, frameNum); });
}
...
function recursiveRender(render, state, endCallback, frameIndex, lastIndex) {
    ...
    if (lastIndex === void 0) { lastIndex = state.length - 1; }
...
function createTargetArray(target, num) {
    return new Array(num).fill(0).map(function (_) { return target; });
}
// eslint-disable-next-line
function updateTargetValues(target, values, key) {
    // eslint-disable-next-line
    target.forEach(function (_, i) { return (_[key] = values[i]); });
    return target;
}
...
function recursiveTransition(startState, endState, frameStateProgress, deep) {
    ...
    if (stateType === 'number')
        return getTransitionState(asNumber(startState), asNumber(endState), frameStateProgress);
...
export { createAnimator, extendCurves, transition };
